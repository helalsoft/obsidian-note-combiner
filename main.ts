...
interface NoteCombinerSettings {
	excludeGlob: string;
...
const DEFAULT_SETTINGS: NoteCombinerSettings = {
	excludeGlob: '',
...
export default class NoteCombinerPlugin extends Plugin {
	settings: NoteCombinerSettings;

	async onload() {
		await this.loadSettings();

		// Ribbon Icon (Optional, maybe not needed if we have commands)
		/*
		...
		*/
		...
		// Command to combine notes from a suggested folder
		this.addCommand({
			id: 'combine-notes-from-folder',
			name: 'Combine notes from folder',
			callback: () => {
		...
		this.addSettingTab(new NoteCombinerSettingTab(this.app, this));
	}
...
class DestinationModal extends Modal {
    ...
    onOpen() {
        ...
        contentEl.createEl("h2", { text: "Destination for Combined Note" });
        ...
        const btn = buttonContainer.createEl("button", { text: "Combine" });
        btn.onclick = () => {
...
class NoteCombinerSettingTab extends PluginSettingTab {
	plugin: NoteCombinerPlugin;

	constructor(app: App, plugin: NoteCombinerPlugin) {
		super(app, plugin);
		this.plugin = plugin;
	}
	...
	display(): void {
		...
		containerEl.createEl('h2', {text: 'Settings for Note Combiner'});
